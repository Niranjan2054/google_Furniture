	...
	if (isset($_POST) && !empty($_POST)) {
		...
		if (isset($_GET['id']) && !empty($_GET['id']) && $_GET['id']>0) {
			...
			if (isset($_GET['act']) && !empty($_GET['act']) && $act == $_GET['act']) {
				...
				$Customers_info = $customer->getCustomerById($customer_id);
				debugger($Customers_info);
				if ($Customers_info) {
					$success = $customer->deleteCustomerbyId($Customers_info[0]->id);
					if ($success) {
						setFlash('../customer','success','Customers Deleted Successfully');
					}else{
						setFlash('../customer','error','Error While Deleting Customers');
					}
				}else{
					setFlash('../customer','error','Customers Not found.');
				}

			}else{
				setFlash('../customer','error','Error unknown access to delete');
			}
		}else{
		...
		setFlash('../customer','error','Unauthorized Access');
	}
?>